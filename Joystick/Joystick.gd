extends TextureRect
###
#THIS SCRIPT IS SHAMELESSLY STOLOEN FROM  https://randommomentania.com/2018/08/godot-touch-screen-joystick-part-1/
...
func _ready():
	...
	print(get_center_of_joystick())
	# Get the inner ring
	joystick_ring = get_node("Joystick_knob");
...
func _draw():
	...
	draw_circle( get_rect().size/2, radius, Color8(256,256,256,128));
			...
			if (joystick_active == false):
				...
				# Otherwise, we just need to check if the event is within the radius of the joystick
					...
					if ( ((get_center_of_joystick()) - event_position).length() <= radius):
						...
						# a normalized vector.
						joystick_vector = ((get_center_of_joystick()) - event_position) / radius;
